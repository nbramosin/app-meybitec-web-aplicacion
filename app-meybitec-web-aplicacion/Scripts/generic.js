...
function Correcto(texto = "Se realizo correctamente", result) {
    const sku = result?.data?.Body?.sku ?? '_';
    //const sku = "CAR03F2622000001";
    Swal.fire({
        position: 'top-end',
        icon: 'success',
        title: texto,
...
/*async function Confirmacion(texto = "Desea guardar los cambios?", title = "Confirmacion",
callback) {
  return  Swal.fire({
        title: title,
        text: texto,
...
async function Confirmacion(texto = "¿Desea guardar los cambios?", title = "Confirmación") {
    ...
    for (var i = 0; i < elementos.length; i++) {
        ...
        if (!excepciones.includes(elementos[i].name)) {
            if (elementos[i].type === "file") {
                elementos[i].value = ""; // limpia archivos (multiple)
            }
        ...
        // Quitar validaciones bootstrap
        elementos[i].classList.remove("is-valid");
        elementos[i].classList.remove("is-invalid");
    }

    // Quitar estado de validación del formulario
    var form = document.getElementById(idFormulario);
    if (form) {
        form.classList.remove("was-validated");
    }

    // Limpiar preview de imágenes múltiples
    var preview = document.getElementById("preview");
    if (preview) {
        // Evitar memory leaks si usas URL.createObjectURL
        preview.querySelectorAll("img").forEach(img => {
            if (img.src.startsWith("blob:")) {
                URL.revokeObjectURL(img.src);
            }
        });
        preview.innerHTML = ""; // elimina todas las imágenes
    }
}
